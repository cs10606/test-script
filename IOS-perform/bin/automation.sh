...
function Run()
{
    ...
    echo "	xcrun instruments -w $device -D $instruments_path/${MODULE_NAME} -t $template $app -e UIARESULTSPATH $instruments_path -e UIASCRIPT $script "
	xcrun instruments \
		-w "$device" \
